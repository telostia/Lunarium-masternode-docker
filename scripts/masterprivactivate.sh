...
while inotifywait -e modify /var/www/masternodeprivkey/masternodeprivkey.txt; do
  ...
  echo "maxconnections=16" >>/root/solarium/solarium.conf
  echo "masternodeprivkey=$MASTERNODEPRIVKEY" >>/root/solarium/solarium.conf
  echo "masternode=1" >>/root/solarium/solarium.conf
  echo "masternodeaddr=$IP:4848" >>/root/solarium/solarium.conf
  docker run -d --name solariummasternode solariummasternode
  docker cp/root/solarium/solarium.conf solariummasternode:/root/.solarium/solarium.conf
  ...
  cd /root/chainfile
  docker cp ./ solariummasternode:~/solarium/
